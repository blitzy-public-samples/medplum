...
/** The only entry configured in `defaultOAuthClients` for this suite. */
...
const OVER_LENGTH_ID_QUERY = '?_id=' + Array.from({ length: DEFAULT_MAX_SEARCH_COUNT + 1 }, () => 'x').join(',');
...
const validationCases: ValidationCase[] = [
  ...
  { name: 'Zero _count is rejected', query: '?_count=0', status: 400 },
  { name: 'Negative _count is rejected', query: '?_count=-1', status: 400 },
  { name: 'Fractional _count is rejected', query: '?_count=1.5', status: 400 },
  { name: 'Non-numeric _count is rejected', query: '?_count=abc', status: 400 },
  { name: 'Repeated _count is rejected', query: REPEATED_COUNT_QUERY, status: 400 },
  {
  ...
  { name: 'Negative _offset is rejected', query: '?_offset=-1', status: 400 },
  { name: 'Fractional _offset is rejected', query: '?_offset=1.5', status: 400 },
  { name: 'Non-numeric _offset is rejected', query: '?_offset=abc', status: 400 },
  { name: 'Repeated _offset is rejected', query: REPEATED_OFFSET_QUERY, status: 400 },
  { name: 'Malformed _id is rejected', query: '?_id=not-a-uuid', status: 400 },
  { name: 'Repeated _id is rejected', query: REPEATED_ID_QUERY, status: 400 },
  { name: 'Over-length _id list is rejected', query: OVER_LENGTH_ID_QUERY, status: 400 },
...
describe('OAuth client security endpoint', () => {
  ...
  describe('Rule outcomes', () => {
    ...
    test('Client with no redirect URI reports OCS-005 and passes', () => {
      ...
      expect(findingsOf(result, 'OCS-005')[0].redirectUri).toBeUndefined();
    });
  });
  ...
  describe('Evaluation options', () => {
    ...
    beforeAll(async () => {
      ...
      // One client colliding with the configured default OAuth client, one with the built-in CLI client.
      ...
      const discoverableRes = await readReport(
        ...
        '?_id=' + configuredCollisionId + ',' + builtInCollisionId
      );
    ...
    test('Client sharing a redirect URI with a configured default OAuth client reports OCS-004', () => {
      ...
      expect(finding.remediation).toContain('default OAuth client configuration');
    });
  ...
  describe('Query parameter validation', () => {
    ...
    test.each(validationCases)('$name', async ({ query, status, count, offset, unfiltered }) => {
